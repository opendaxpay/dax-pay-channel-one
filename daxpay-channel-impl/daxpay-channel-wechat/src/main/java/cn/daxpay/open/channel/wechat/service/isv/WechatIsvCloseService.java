...
/// # 微信服务商通道关闭服务
///
/// 调用微信 V3 服务商 `关闭订单` 接口(closePartnerOrderV3)终结未支付订单。
/// 失败兜底(同直连): ORDER_NOT_EXIST / ORDER_CLOSED 视为关单成功。
@Slf4j
@Service
public class WechatIsvCloseService {
    ...
    public WechatCloseResp close(WechatCloseReq req) {
        ...
        try {
            service.closePartnerOrderV3(req.getOutTradeNo());
            return toResp(req);
        } catch (WxPayException e) {
            // 兜底: 订单不存在 / 订单已关闭 视为关单成功
            String errCode = e.getErrCode();
            if (ORDER_NOT_EXIST.equals(errCode) || ORDER_CLOSED.equals(errCode)) {
                log.info("微信服务商关单兜底成功: outTradeNo={}, errCode={}", req.getOutTradeNo(), errCode);
                return toResp(req);
            }
            log.error("微信服务商关闭订单失败: outTradeNo={}, errCode={}, errMsg={}",
