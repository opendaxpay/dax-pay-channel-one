...
/// # 支付宝通道退款响应
///
/// 子应用调用 `alipay.trade.refund` 后回传给主应用。
/// `complete=false` 表示资金未变动(fund_change=N), 需主应用经退款同步查询确认最终状态;
...
public class AlipayRefundResp {
    ...
    /// 是否已终态完成(true 表示 fund_change=Y 退款即时成功, 无需再同步查询)
    private Boolean complete;

    /// 退款完成时间(gmt_refund_pay)
    private OffsetDateTime finishTime;

    /// 退款金额(单位: 分, 由网关 refund_amount 元转分)
